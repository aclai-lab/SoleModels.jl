@test_nowarn SoleModels.parsecondition("min[1] <= 32")
@test_nowarn SoleModels.parsecondition("max[2] <= 435")
@test_nowarn SoleModels.parsecondition("minimum[6]    > 250.631")
@test_nowarn SoleModels.parsecondition("   minimum   [7]    > 11.2")
@test_nowarn SoleModels.parsecondition("avg [8]    > 63.2  ")
@test_nowarn SoleModels.parsecondition("mean[9] <= 1.0e100")

@test_nowarn SoleModels.parsecondition("max{3] <= 12", opening_bracket="{")
@test_nowarn SoleModels.parsecondition("  min[4}    > 43.25  ", closing_bracket="}")
@test_nowarn SoleModels.parsecondition(
    "max{5} <= 250", opening_bracket="{", closing_bracket="}")
@test_nowarn SoleModels.parsecondition("mean[9] <= 1.0e100")
@test_nowarn SoleModels.parsecondition(
    "mean🌅9🌄 <= 1.0e100", opening_bracket="🌅", closing_bracket="🌄")

@test_nowarn SoleModels.featvaltype(SoleModels.feature(
    SoleModels.parsecondition("mean[10]    > 462.2"; featvaltype = Float64))) == Float64
@test_nowarn SoleModels.featvaltype(SoleModels.feature(
