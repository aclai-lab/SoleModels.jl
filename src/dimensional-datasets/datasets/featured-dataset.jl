...
# function fwd_init_world_slice(fwd::GenericFWD{V}, i_instance::Integer, w::AbstractWorld) where {V}
#     fwd.d[i_instance][w] = Array{V,1}(undef, fwd.nfeatures)
# end
...
#     i_instance    :: Integer,
#     w           :: AbstractWorld,
#     i_feature   :: Integer) where {V} = fwd.d[i_instance][w][i_feature]

# # A function for setting a threshold value in the lookup table
# Base.@propagate_inbounds @inline function fwd_set(fwd::GenericFWD{V}, w::AbstractWorld, i_instance::Integer, i_feature::Integer, threshold::V) where {V}
...
# Others...
# Base.@propagate_inbounds @inline fwdread_channeaoeu(fwd::GenericFWD{V}, i_instance::Integer, i_feature::Integer) where {V} = TODO
...
struct FeaturedDataset{
    ...
    Base.@propagate_inbounds function FeaturedDataset(
        ...
        fwd = begin
            ...
            @inbounds Threads.@threads for i_instance in 1:_n_samples
                # @logmsg LogDebug "Instance $(i_instance)/$(_n_samples)"

                # if i_instance == 1 || ((i_instance+1) % (floor(Int, ((_n_samples)/4))+1)) == 0
                #     @logmsg LogOverview "Instance $(i_instance)/$(_n_samples)"
                # end
                ...
                for w in allworlds(X, i_instance)
                    ...
                    for (i_feature,feature) in enum_features
                        ...
                        fwd[w, i_instance, i_feature] = gamma
...
@inline function Base.getindex(
    ...
    # X[i_instance, w, feature, i_feature, args...]::V
    fwd(X)[i_instance, w, i_feature, args...]::V
end
...
frame(X::FeaturedDataset, i_instance) = frame(fwd(X), i_instance)
initialworld(X::FeaturedDataset) = X.initialworld
function initialworld(X::FeaturedDataset, i_instance)
