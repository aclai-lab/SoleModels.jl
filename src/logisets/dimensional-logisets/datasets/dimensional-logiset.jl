...
instances(X::DimensionalLogiset, inds::AbstractVector{<:Integer}, args...; kwargs...)    =
    DimensionalLogiset(instances(domain(X), inds, args...; kwargs...), ontology(X), features(X), grouped_featsaggrsnops(X); initialworld = initialworld(X))

frame(X::DimensionalLogiset, i_instance::Integer) = frame(domain(X), i_instance)
initialworld(X::DimensionalLogiset) = X.initialworld
function initialworld(X::DimensionalLogiset, i_instance::Integer)
