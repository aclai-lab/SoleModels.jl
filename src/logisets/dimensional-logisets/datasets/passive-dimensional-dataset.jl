...
} <: AbstractLogiset{W,FT where FT<:VarFeature,Bool,FR} # Note: truth value could by different
...
instances(X::PassiveDimensionalDataset{N,W}, inds::AbstractVector{<:Integer}, args...; kwargs...) where {N,W} =
    PassiveDimensionalDataset{N,W}(instances(X.d, inds, args...; kwargs...))

hasnans(X::PassiveDimensionalDataset) = hasnans(X.d)

worldtype(X::PassiveDimensionalDataset{N,W}) where {N,W} = W

frame(X::PassiveDimensionalDataset, i_instance::Integer) = _frame(X.d, i_instance)
