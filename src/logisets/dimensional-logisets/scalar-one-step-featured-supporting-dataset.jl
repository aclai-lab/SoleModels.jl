...
struct OneStepFeaturedMemoset{
    V<:Number,
    W<:AbstractWorld,
    FR<:AbstractFrame{W},
    VV<:Union{V,Nothing},
    FWDRS<:AbstractScalarOneStepRelationalMemoset{VV,W,FR},
    FWDGS<:Union{AbstractScalarOneStepGlobalMemoset{W,V},Nothing},
    G<:AbstractVector{Tuple{<:AbstractFeature,<:Aggregator}},
} <: FeaturedMemoset{V,W,FR}
    ...
    function OneStepFeaturedMemoset(
        fwd_rs::FWDRS,
        fwd_gs::FWDGS,
        featsnaggrs::G,
    ) where {
    ...
    _default_rs_type(::Type{<:AbstractWorld}) = ScalarOneStepRelationalMemoset
    ...
    Base.@propagate_inbounds function OneStepFeaturedMemoset(
        fd                     :: Logiset{V,W},
        relational_support_type :: Type{<:AbstractScalarOneStepRelationalMemoset} = _default_rs_type(W);
        ...
        OneStepFeaturedMemoset(fwd_rs, fwd_gs, featsnaggrs)
    end
end

fwd_rs(X::OneStepFeaturedMemoset) = X.fwd_rs
fwd_gs(X::OneStepFeaturedMemoset) = X.fwd_gs
featsnaggrs(X::OneStepFeaturedMemoset) = X.featsnaggrs

ninstances(X::OneStepFeaturedMemoset) = ninstances(fwd_rs(X))
# nfeatsnaggrs(X::OneStepFeaturedMemoset) = nfeatsnaggrs(fwd_rs(X))
...
usesmemo(X::OneStepFeaturedMemoset) = usesglobalmemo(X) || usesmodalmemo(X)
usesglobalmemo(X::OneStepFeaturedMemoset) = false
usesmodalmemo(X::OneStepFeaturedMemoset) = usesmemo(fwd_rs(X))

Base.size(X::OneStepFeaturedMemoset) = (size(fwd_rs(X)), (isnothing(fwd_gs(X)) ? () : size(fwd_gs(X))))

find_featsnaggr_id(X::OneStepFeaturedMemoset, feature::AbstractFeature, aggregator::Aggregator) = findfirst(x->x==(feature, aggregator), featsnaggrs(X))

function instances(X::OneStepFeaturedMemoset, inds::AbstractVector{<:Integer}, args...; kwargs...)
    OneStepFeaturedMemoset(
        instances(fwd_rs(X), inds, args...; kwargs...),
        (isnothing(fwd_gs(X)) ? nothing : instances(fwd_gs(X), inds, args...; kwargs...)),
        featsnaggrs(X)
    )
end


function hasnans(X::OneStepFeaturedMemoset)
    hasnans(fwd_rs(X)) || (!isnothing(fwd_gs(X)) && hasnans(fwd_gs(X)))
end

isminifiable(X::OneStepFeaturedMemoset) = isminifiable(fwd_rs(X)) && (isnothing(fwd_gs(X)) || isminifiable(fwd_gs(X)))

function minify(X::OSSD) where {OSSD<:OneStepFeaturedMemoset}
...
function displaystructure(X::OneStepFeaturedMemoset; indent_str = "")
