...
computeModalThresholdDual(test_operator::CanonicalFeatureGeq, w::Interval{Int}?, r::RCC8RelationFromIA, channel::AbstractArray{T,1}) where {T} = begin
    maxExtrema(
        map((IA_r)->(yieldReprs(test_operator, enum_acc_repr(test_operator, w, IA_r, length(channel)), channel)), topo2IARelations(r))
    )
end
apply_aggregator(test_operator::CanonicalFeatureGeq, w::Interval{Int}?, r::RCC8RelationFromIA, channel::AbstractArray{T,1}) where {T} = begin
    maximum(
        map((IA_r)->(yieldRepr(test_operator, enum_acc_repr(test_operator, w, IA_r, length(channel)), channel)), topo2IARelations(r))
    )
end
apply_aggregator(test_operator::CanonicalFeatureLeq, w::Interval{Int}?, r::RCC8RelationFromIA, channel::AbstractArray{T,1}) where {T} = begin
    mininimum(
        map((IA_r)->(yieldRepr(test_operator, enum_acc_repr(test_operator, w, IA_r, length(channel)), channel)), topo2IARelations(r))
    )
end
...
computeModalThresholdDual(test_operator::CanonicalFeatureGeq, w::Interval{Int}?, r::RCC5Relation, channel::AbstractArray{T,1}) where {T} = begin
    maxExtrema(
...
apply_aggregator(test_operator::CanonicalFeatureGeq, w::Interval{Int}?, r::RCC5Relation, channel::AbstractArray{T,1}) where {T} = begin
    maximum(
...
apply_aggregator(test_operator::CanonicalFeatureLeq, w::Interval{Int}?, r::RCC5Relation, channel::AbstractArray{T,1}) where {T} = begin
    mininimum(
        map((IA_r)->(yieldRepr(test_operator, enum_acc_repr(test_operator, w, IA_r, size(channel)...), channel)), [IA_r for RCC8_r in RCC52RCC8Relations(r) for IA_r in topo2IARelations(RCC8_r)])
    )
end
