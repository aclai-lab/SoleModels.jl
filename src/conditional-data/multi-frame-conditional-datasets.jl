...
getframe(X::MultiFrameConditionalDataset, i_frame::Integer)                = nframes(X) > 0 ? frames(X)[i_frame] : error("MultiFrameConditionalDataset has no frame!")
nframes(X::MultiFrameConditionalDataset)                                   = length(frames(X))
nsamples(X::MultiFrameConditionalDataset)                                  = nsamples(getframe(X, 1))
...
nfeatures(X::MultiFrameConditionalDataset,  i_frame::Integer) = nfeatures(getframe(X, i_frame))
nrelations(X::MultiFrameConditionalDataset, i_frame::Integer) = nrelations(getframe(X, i_frame))
worldtype(X::MultiFrameConditionalDataset,  i_frame::Integer) = worldtype(getframe(X, i_frame))
worldtypes(X::MultiFrameConditionalDataset) = Vector{Type{<:AbstractWorld}}(worldtype.(frames(X)))

get_instance(X::MultiFrameConditionalDataset,  i_frame::Integer, idx_i::Integer, args...)  = get_instance(getframe(X, i_frame), idx_i, args...)
# get_instance(X::MultiFrameConditionalDataset, idx_i::Integer, args...)  = get_instance(getframe(X, i), idx_i, args...) # TODO should slice across the frames!
