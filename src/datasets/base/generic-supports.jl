...
struct GenericRelationalSupport{
    ...
    function GenericRelationalSupport(fd::Logiset{V,W,FR}, perform_initialization = false) where {V,W,FR<:AbstractFrame{W,Bool}}
...
struct GenericGlobalSupport{V,D<:AbstractArray{V,2}} <: AbstractGlobalSupport{V}
    ...
    function GenericGlobalSupport(fd::Logiset{V}) where {V}
