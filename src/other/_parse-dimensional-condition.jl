...
"""
    parsecondition(
        ::Type{ScalarCondition{VarFeature}},
        expression::String;
...
# Examples
```julia-repl
julia> syntaxstring(SoleModels.parsecondition(ScalarCondition{VarFeature}, "min[V1] <= 32"))
"min[V1] <= 32.0"

julia> syntaxstring(parseformulatree("min[V1] <= 15 ∧ max[V1] >= 85"; proposition_parser=(x)->parsecondition(ScalarCondition{VarFeature}, x; featvaltype = Int64,)))
"min[V1] <= 15 ∧ max[V1] >= 85"
```
...
function parsecondition(
    ...
    (_feature, _variable, _test_operator, _threshold) = begin
        ...
        @assert !isnothing(slices) && length(slices) == 4 "Could not parse condition from " *
            "expression $(repr(expression))."
    ...
    return ScalarCondition(feature, test_operator, threshold)
end
