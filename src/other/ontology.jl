...
world2frametype = Dict([
    OneWorld => FullDimensionalFrame{0,OneWorld},
    Interval => FullDimensionalFrame{1,Interval{Int}},
    Interval2D => FullDimensionalFrame{2,Interval2D{Int}},
])
...
struct Ontology{W<:AbstractWorld}
    ...
    function Ontology{W}(_relations::AbstractVector) where {W<:AbstractWorld}
        ...
        #     @assert goeswith(world2frametype[W], relation) "Cannot instantiate Ontology{$(W)} with relation $(relation)!"
        # end
